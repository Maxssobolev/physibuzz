...
export default function Home() {
  ...
  const getData = (url, pageNumber = 1) => {
    api.get(`${url}?page=${pageNumber}`).then((r) => {
      let recievedData = url.includes('vacancies') ? r.data.data : r.data[0] //need to change
  ...
  useEffect(() => {
    ...
    //если метоположение выставлено
      ...
      /*
      if (jobOrCourseSelected.label == 'Job') {
        getData('api/v1/vacancies/search?&country=United States')
      }
      else if (jobOrCourseSelected.label == 'Course') {
        getData('api/v1/courses/search?&country=United States')
      }
*/
    }
